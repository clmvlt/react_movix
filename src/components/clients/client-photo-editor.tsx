...
const WIDTH_LABELS = [
  "clients.photos.editor.widthSmall",
  "clients.photos.editor.widthMedium",
  "clients.photos.editor.widthLarge",
];

interface ClientPhotoEditorProps {
  clientId: string;
  picture: ClientPicture | null;
  open: boolean;
...
export function ClientPhotoEditor({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const blob = await clientsApi.pictureBlob(imagePath);
        ...
        if (!cancelled) setError(t("clients.photos.editor.errors.load"));
      } finally {
  ...
  const handleSave = () => {
    ...
    try {
      ...
      toast.error(t("clients.photos.editor.errors.save"));
      return;
    }

    if (dataUrlBytes(base64) > PICTURE_MAX_BYTES) {
      toast.error(t("clients.photos.editor.errors.tooLarge"));
      return;
    }
    ...
    updatePicture.mutate(
      { id: clientId, pictureId: picture.id, input: { base64 } },
      {
        ...
        onError: () => toast.error(t("clients.photos.editor.errors.save")),
      }
    );
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex h-[calc(100dvh-2rem)] max-w-[calc(100%-1rem)] flex-col gap-3 sm:max-w-5xl">
        <DialogHeader className="shrink-0">
          <DialogTitle>{t("clients.photos.editor.title")}</DialogTitle>
          <DialogDescription>
            {t("clients.photos.editor.subtitle")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          <div className="flex shrink-0 gap-1 rounded-lg border bg-card p-1">
            ...
            >
              <Type />
              {t("clients.photos.editor.text")}
            </Button>
          </div>

          <div
            role="group"
            aria-label={t("clients.photos.editor.color")}
          ...
          {tool === "pen" && (
            <div
              role="group"
              aria-label={t("clients.photos.editor.width")}
          ...
          <div className="flex shrink-0 gap-1 sm:ml-auto">
            <Button
              ...
              title={t("clients.photos.editor.undo")}
              aria-label={t("clients.photos.editor.undo")}
              className="min-h-10"
            >
              <RotateCcw />
              <span className="hidden lg:inline">
                {t("clients.photos.editor.undo")}
              </span>
            </Button>
            <Button
              type="button"
              ...
              disabled={annotations.length === 0}
              title={t("clients.photos.editor.clear")}
              aria-label={t("clients.photos.editor.clear")}
              className="min-h-10"
            >
              <Trash2 />
              <span className="hidden lg:inline">
                {t("clients.photos.editor.clear")}
              </span>
            </Button>
          </div>
        </div>
        ...
        {tool === "text" && (
          <div className="flex shrink-0 flex-col gap-1.5">
            <Input
              ...
              placeholder={t("clients.photos.editor.textPlaceholder")}
              aria-label={t("clients.photos.editor.textPlaceholder")}
              className="min-h-11 lg:min-h-10"
            />
            <p className="text-xs text-muted-foreground">
              {t("clients.photos.editor.textHint")}
            </p>
          </div>
        )}
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-xl border bg-neutral-900 p-2 sm:p-4">
          {size ? (
            ...
            <span className="flex items-center gap-2 p-6 text-sm text-neutral-200">
              ...
              {loading ? t("clients.photos.editor.loading") : ""}
            </span>
          )}
        </div>
