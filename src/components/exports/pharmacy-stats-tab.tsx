...
import {
  ...
  useStatsByPharmacy,
  useStatsOverview,
  type PharmacyStatsParams,
  type StatsPharmacyItem,
} from "@/features/stats";
...
const ACCESSORS: Record<string, (row: StatsPharmacyItem) => string | number> = {
  cip: (row) => row.cip,
...
export function PharmacyStatsTab({ api }: { api: ExportFiltersApi }) {
  const { t } = useTranslation();
  const { filters, setFilter, rangeError } = api;

  const [params, setParams] = useState<PharmacyStatsParams | null>(null);
  ...
  const byPharmacyQuery = useStatsByPharmacy(statsFilters);

  const isLoading =
    params !== null && (overviewQuery.isPending || byPharmacyQuery.isPending);
  const isError = overviewQuery.isError || byPharmacyQuery.isError;

  const pharmacies = byPharmacyQuery.data?.pharmacies ?? [];
  const overview = overviewQuery.data ?? null;

  useEffect(() => {
    if (!byPharmacyQuery.data) return;
    setSelectedCips(byPharmacyQuery.data.pharmacies.map((row) => row.cip));
    if (startedAtRef.current !== null) {
      setDuration(Math.round(performance.now() - startedAtRef.current));
      startedAtRef.current = null;
    }
  }, [byPharmacyQuery.data]);

  const selectedPharmacies = pharmacies.filter((row) =>
    selectedCips.includes(row.cip)
  );
  const isAllSelected =
    pharmacies.length > 0 && selectedCips.length === pharmacies.length;
  const totals = computePharmacyTotals(
    selectedPharmacies,
    overview,
    isAllSelected
  );
  ...
  const toggleAll = () => {
    setSelectedCips(isAllSelected ? [] : pharmacies.map((row) => row.cip));
  };

  const togglePharmacy = (cip: string) => {
  ...
  const notices = hasData
    ? [
        totals.isLocal ? t("exports.pharmacies.localTotals") : null,
        lowRateCount > 0
          ? t("exports.pharmacies.lowRate", {
              count: lowRateCount,
    ...
    <div className="flex flex-1 flex-col gap-3">
      <Card>
        <CardContent className="flex flex-col gap-3 p-3 sm:p-4">
          ...
          <p className="text-xs text-muted-foreground">
            {t("exports.pharmacies.scopeHint")}
          </p>
        </CardContent>
      </Card>
      ...
      {coherenceIssue && (
        <Alert variant="destructive">
          <TriangleAlert />
          <AlertDescription>
            {t("exports.pharmacies.coherence", {
      ...
      {isLoading && (
        <div className="flex min-h-[280px] flex-1 flex-col items-center justify-center gap-3 rounded-xl border border-dashed p-6 text-center">
          ...
          <p className="text-sm font-medium">
            {t("exports.pharmacies.loadingTitle")}
          </p>
        </div>
      )}
      ...
      {!isLoading && !isError && params === null && (
        <EmptyState
          message={t("exports.pharmacies.idle")}
      ...
      {!isLoading && !isError && params !== null && !hasData && (
        <EmptyState
          message={t("exports.pharmacies.empty")}
          className="min-h-[280px] flex-1"
        />
      )}
          ...
          <Card>
            <CardContent className="p-0">
              <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex min-w-0 flex-col gap-0.5">
                  <p className="text-sm font-semibold">
                    {t("exports.pharmacies.tableTitle", {
                      shown: visibleRows.length,
                      total: pharmacies.length,
                    })}
                  </p>
                  <span className="text-xs text-muted-foreground">
                    {t("exports.pharmacies.selectedCount", {
                ...
                <Input
                  value={search}
                  placeholder={t("exports.pharmacies.searchPlaceholder")}
                  aria-label={t("exports.pharmacies.searchPlaceholder")}
                  ...
                  <div className="min-w-[1120px]">
                    ...
                    >
                      {virtualizer.getVirtualItems().map((virtualRow) => {
                        ...
                        const selected = selectedCips.includes(row.cip);
                        const lowRate =
                        ...
                        return (
                          <button
                            ...
                            onClick={() => togglePharmacy(row.cip)}
                            className={cn(
              ...
              <p className="border-t border-border px-3 py-2 text-[11px] text-muted-foreground">
                {t("exports.pharmacies.tableNote")}
              </p>
            </CardContent>
          </Card>
        </>
      ...
      <SheetPreviewDialog
        ...
        title={t("exports.preview.pharmaciesTitle")}
