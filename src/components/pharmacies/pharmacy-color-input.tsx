...
interface PharmacyColorInputProps {
  id: string;
  value: string;
  onChange: (value: string) => void;
}

export function PharmacyColorInput({
  id,
  value,
  onChange,
}: PharmacyColorInputProps) {
  ...
  return (
    <div className="flex min-h-11 items-center gap-2 lg:min-h-10">
      ...
      <span className="min-w-0 flex-1 truncate text-sm uppercase text-muted-foreground">
        {trimmed || t("pharmacies.info.noColor")}
      </span>
      {trimmed && (
        <Button
          type="button"
          ...
          onClick={() => onChange("")}
          aria-label={t("pharmacies.info.clearColor")}
        >
          <X className="size-4" />
        </Button>
      )}
