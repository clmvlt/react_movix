...
import { PharmacyPhotoUploader } from "@/components/pharmacies/pharmacy-photo-uploader";
import { PharmacyPhotoLightbox } from "@/components/pharmacies/pharmacy-photo-lightbox";
import { imageUrl } from "@/lib/images";
import { cn } from "@/lib/utils";
import { useToast } from "@/app/toast-context";
import {
  sortPictures,
  useDeletePharmacyPicture,
  useReorderPharmacyPictures,
  type PharmacyPicture,
} from "@/features/pharmacies";

interface PharmacyPhotoGalleryProps {
  cip: string;
  pictures: PharmacyPicture[];
}

export function PharmacyPhotoGallery({
  cip,
  pictures,
}: PharmacyPhotoGalleryProps) {
  const { t } = useTranslation();
  const toast = useToast();
  const reorder = useReorderPharmacyPictures();
  const removePicture = useDeletePharmacyPicture();
  ...
  const [deleteTarget, setDeleteTarget] = useState<PharmacyPicture | null>(null);
  ...
  const ordered = useMemo(() => {
    ...
    return localIds
      .map((id) => byId.get(id))
      .filter((picture): picture is PharmacyPicture => Boolean(picture));
  }, [localIds, sorted]);
  ...
  const handleSaveOrder = () => {
    reorder.mutate(
      { cip, ids: ordered.map((picture) => picture.id) },
      {
        onSuccess: () => setLocalIds(null),
        onError: () => toast.error(t("pharmacies.photos.errors.order")),
      }
    );
  };
  ...
  const handleDelete = () => {
    ...
    removePicture.mutate(
      { cip, id: deleteTarget.id },
      {
        onSuccess: () => {
          setDeleteTarget(null);
          setLightboxIndex(null);
        },
        onError: () => toast.error(t("pharmacies.photos.errors.delete")),
      }
    );
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <PharmacyPhotoUploader cip={cip} nextDisplayOrder={nextDisplayOrder} />

      {dirty && (
        <div className="flex flex-col gap-2 rounded-xl border p-3 sm:flex-row sm:items-center">
          <p className="min-w-0 flex-1 text-sm text-muted-foreground">
            {t("pharmacies.photos.orderChanged")}
          </p>
          <div className="flex gap-2">
            ...
            >
              {t("pharmacies.photos.cancelOrder")}
            </Button>
            <Button
              type="button"
              className="min-h-11 flex-1 sm:min-h-10 sm:flex-none"
      ...
      {ordered.length === 0 ? (
        <EmptyState message={t("pharmacies.photos.empty")} />
      ) : (
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-label={
                    picture.originalName ?? t("pharmacies.photos.openLightbox")
                  }
                ...
                <div className="flex items-center justify-between gap-1 border-t p-1">
                  <Button
                    ...
                    disabled={position === 0}
                    title={t("pharmacies.photos.moveUp")}
                    aria-label={t("pharmacies.photos.moveUp")}
                  >
                  ...
                  <Button
                    ...
                    className="size-11 shrink-0 lg:size-8"
                    onClick={() => setLightboxIndex(position)}
                    title={t("pharmacies.photos.fullscreen")}
                    aria-label={t("pharmacies.photos.fullscreen")}
                  >
                  ...
                  <Button
                    ...
                    title={t("pharmacies.photos.moveDown")}
                    aria-label={t("pharmacies.photos.moveDown")}
                  >
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t("pharmacies.photos.deleteTitle")}</DialogTitle>
            <DialogDescription>
              {t("pharmacies.photos.deleteConfirm")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
