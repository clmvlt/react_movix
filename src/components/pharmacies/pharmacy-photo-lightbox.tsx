...
import { PharmacyPhotoEditor } from "@/components/pharmacies/pharmacy-photo-editor";
...
interface PharmacyPhotoLightboxProps {
  cip: string;
  pictures: PharmacyPicture[];
  index: number | null;
  onIndexChange: (index: number | null) => void;
  onDelete: (picture: PharmacyPicture) => void;
}

export function PharmacyPhotoLightbox({
  cip,
  pictures,
  index,
  onIndexChange,
  onDelete,
}: PharmacyPhotoLightboxProps) {
  ...
  const handleReplace = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith("image/") || file.size > PICTURE_MAX_BYTES) {
      toast.error(t("pharmacies.photos.tooLarge", { name: file.name }));
      return;
    }
    ...
    updatePicture.mutate(
      { cip, id: picture.id, input: { base64 } },
      {
        ...
        onError: () => toast.error(t("pharmacies.photos.errors.replace")),
      }
    );
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          <DialogHeader>
            <DialogTitle>{t("pharmacies.photos.title")}</DialogTitle>
            <DialogDescription>
          ...
          <div className="flex min-h-0 flex-1 items-center gap-2">
            <Button
              ...
              onClick={() => go(-1)}
              disabled={index === 0}
              title={t("pharmacies.photos.previous")}
              aria-label={t("pharmacies.photos.previous")}
            >
              <ChevronLeft />
            </Button>
            ...
            <div className="flex min-h-0 min-w-0 flex-1 items-center justify-center overflow-hidden rounded-xl border bg-neutral-900 p-2 sm:p-4">
              {broken ? (
                <div className="flex flex-col items-center gap-2 p-6 text-center text-neutral-200">
                  ...
                  <p className="text-sm">{t("pharmacies.photos.broken")}</p>
                  <p className="text-xs text-neutral-400">
                    {t("pharmacies.photos.brokenHint")}
                  </p>
                </div>
              ) : (
                <img
            ...
            <Button
              ...
              title={t("pharmacies.photos.next")}
              aria-label={t("pharmacies.photos.next")}
            >
              <ChevronRight />
            </Button>
          </div>
