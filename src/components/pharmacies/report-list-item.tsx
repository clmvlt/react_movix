...
export function ReportListItem({
  ...
  const pharmacyName =
    report.pharmacy?.name?.trim() || t("pharmacies.reports.noPharmacy");
  const location = [report.pharmacy?.postalCode, report.pharmacy?.city]
    .filter(Boolean)
    .join(" ");
          ...
          <span className="flex flex-wrap items-center gap-1.5">
            ...
            {report.pictures.length > 0 && (
              <Badge variant="secondary">
                ...
                {t("pharmacies.reports.photoCount", {
