...
import { ReportWorkspacePanel } from "@/components/reports/report-workspace-panel";
import { cn } from "@/lib/utils";
import { useClientReports } from "@/features/client-reports";
...
export function ClientReportsPage() {
  ...
  const reportsQuery = useClientReports();
  ...
  const reports = useMemo(() => {
    ...
    return (reportsQuery.data ?? []).filter((report) => {
      ...
      const haystack = [
        report.client?.name,
        report.client && "cip" in report.client ? report.client.cip : null,
        report.client?.city,
  ...
  const viewItems: ViewSwitchItem<ReportsView>[] = [
    {
      value: "list",
      label: t("clientReports.views.list"),
      icon: ClipboardList,
      count: reports.length,
    },
    {
      value: "detail",
      label: t("clientReports.views.detail"),
      icon: Wrench,
    },
  ];
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 flex-col">
      <PageHeader
        title={t("clientReports.title")}
        subtitle={t("clientReports.subtitle")}
      />
      ...
      <div className="flex min-h-0 flex-1 gap-4">
        ...
        >
          <div className="relative shrink-0">
            ...
            <Input
              ...
              placeholder={t("clientReports.filterPlaceholder")}
          ...
          {reportsQuery.isLoading ? (
            ...
            <EmptyState
              message={t("clientReports.empty")}
              icon={<ClipboardList className="size-8" />}
            />
          ) : (
            <>
              <p className="shrink-0 text-sm text-muted-foreground">
                {t("clientReports.count", { count: reports.length })}
              </p>
        ...
        >
          {selectedReport ? (
            ...
            <EmptyState
              message={t("clientReports.reportGone")}
              className="flex-1"
            />
          ) : (
            <EmptyState
              message={t("clientReports.selectPrompt")}
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t("clientReports.discardTitle")}</DialogTitle>
            <DialogDescription>
              {t("clientReports.discardConfirm")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
